 ...
 * Spacer stays in layout flow; the bar is portaled to document.body so
 * Lenis / transformed ancestors cannot break position:fixed or z-index.
 */
export function HardwareStoreHeader({
  ...
  const bar = (
    <div className="hardware-store-bar fixed start-0 end-0 h-12 border-b border-line bg-bg/95 backdrop-blur-md supports-[backdrop-filter]:bg-bg/85">
      <div className="shell flex h-full items-center justify-between gap-2 sm:gap-3">
        <div className="relative min-w-0 flex-1">
          <div className="flex items-center gap-0.5 min-w-0 overflow-x-auto pe-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <span className="hidden sm:inline font-mono text-[0.65rem] uppercase tracking-[0.16em] text-muted pe-3 border-e border-line me-1.5 shrink-0">
              {h.eyebrow}
            </span>
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={full}
                  className={`shrink-0 inline-flex items-center min-h-9 h-9 px-3 rounded-full text-sm transition-colors ${
                    active
                      ? "bg-ink text-bg"
          ...
          <div
            className="pointer-events-none absolute inset-y-0 end-0 w-8 bg-gradient-to-l from-bg to-transparent"
            aria-hidden
          />
        </div>
  ...
  return (
    <>
      <div className="hardware-store-spacer" aria-hidden />
